...
import tools.annotation.dextr.networks.deeplab_resnet_app as resnet
from tools.annotation.dextr.mypath import Path
from tools.annotation.dextr.dataloaders import helpers as helpers
...
def dextr():
    ...
    image = np.array(Image.open('tools/annotation/dextr/ims/dog-cat.jpg'))
    plt.ion()
    plt.axis('off')
    plt.imshow(image)
    plt.title(
    ...
    with torch.no_grad():
        while 1:
            extreme_points_ori = np.array(
                plt.ginput(4, timeout=0)).astype(np.int)
